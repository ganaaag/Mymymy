...
async def set_log_channel_cmd(client: Client, message: Message):
    ...
    try:
        ...
        if not db.is_admin(message.from_user.id):
            await message.reply_text("鈿狅笍 You are not authorized to use this command.")
            return
        ...
        try:
            ...
            await message.reply_text("鉂� Invalid channel ID. Please use a valid number.")
            return

        # Set the log channel without validation
        if db.set_log_channel(client.me.username, channel_id):
            await message.reply_text(
                "鉁� Log channel set successfully!\n\n"
                f"Channel ID: {channel_id}\n"
                f"Bot: @{client.me.username}"
            )
        else:
            await message.reply_text("鉂� Failed to set log channel. Please try again.")

    except Exception as e:
        await message.reply_text(f"鉂� Error: {str(e)}")
    ...
    try:
        ...
        if channel_id:
            ...
            try:
                ...
                channel_info = f"馃摙 Channel Name: {channel.title}\n"
            except:
                channel_info = ""
            
            await message.reply_text(
                f"**馃搵 Log Channel Info**\n\n"
                f"馃 Bot: @{client.me.username}\n"
                f"{channel_info}"
                f"馃啍 Channel ID: `{channel_id}`\n\n"
...
BUTTONSCONTACT = InlineKeyboardMarkup([[InlineKeyboardButton(text="馃摓 Contact", url="https://t.me/ItsUGxBot")]])
keyboard = InlineKeyboardMarkup(
    [
        [
            InlineKeyboardButton(text="馃洜锔� Help", url="https://t.me/ItsUGBot")        ],
    ]
)
...
async def restart_handler(_, m):
    ...
    await m.reply_text("馃殾**STOPPED**", True)
...
async def start(bot: Client, m: Message):
    try:
        if m.chat.type == "channel":
            ...
            await m.reply_text(
                "**鉁� Bot is active in this channel**\n\n"
                "**Available Commands:**\n"
                "鈥� /drm - Download DRM videos\n"
                "鈥� /plan - View channel subscription\n\n"
            ...
            if not is_authorized:
                await m.reply_photo(
                    photo=photologo,
                    caption="**馃敀 Access Required**\n\nContact admin to get access.",
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("馃挮 Get Access", url="https://t.me/ItsUGBot")]
                    ])
                )
                return
                
            commands_list = (
                "**馃 Available Commands**\n\n"
                "鈥� /drm - Start Uploading...\n"
                "鈥� /plan - View subscription\n"
            )
            
            if is_admin:
                commands_list += (
                    "\n**馃憫 Admin Commands**\n"
                    "鈥� /users - List all users\n"
                )
            
            await m.reply_photo(
                photo=photologo,
                caption=f"**馃憢 Welcome {m.from_user.first_name}!**\n\n{commands_list}",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("馃摎 Help", url="https://t.me/ItsUGBot")]])
            )
...
async def txt_handler(bot: Client, m: Message):  
    ...
    if m.chat.type == "channel":
        ...
        if not db.is_user_authorized(m.from_user.id, bot_username):
            await m.reply_text("鉂� You are not authorized to use this command.")
            return
    ...
    if not input.document:
        await m.reply_text("<b>鉂� Please send a text file!</b>")
        return
        
    # Check if it's a text file
    if not input.document.file_name.endswith('.txt'):
        await m.reply_text("<b>鉂� Please send a .txt file!</b>")
        return
    ...
    try:    
        ...
        await m.reply_text("<b>鉂� File encoding error! Please make sure the file is saved with UTF-8 encoding.</b>")
        os.remove(x)
        return
    except Exception as e:
        await m.reply_text(f"<b>馃敼Error reading file: {str(e)}</b>")
        os.remove(x)
        return
    
    await editable.edit(
    f"**Total 馃敆 links found are {len(links)}\n"
    ...
    chat_id = editable.chat.id
    timeout_duration = 3 if auto_flags.get(chat_id) else 20
    await editable.edit("**Send Video Thumbnail:**\n\n鈥� Send Photo for custom thumbnail\n鈥� Send /d for default thumbnail\n鈥� Send /skip to skip")
    ...
    try:
        ...
        if input6.photo:
            ...
            try:
                ...
                thumb = temp_file
                await editable.edit("**鉁� Custom thumbnail saved successfully!**")
                await asyncio.sleep(1)
            except Exception as e:
                print(f"Error downloading thumbnail: {str(e)}")
                await editable.edit("**鈿狅笍 Failed to save thumbnail! Using default.**")
                thumb = "/d"
                await asyncio.sleep(1)
        elif input6.text:
            if input6.text == "/d":
                thumb = "/d"
                await editable.edit("**鈩癸笍 Using default thumbnail.**")
                await asyncio.sleep(1)
            elif input6.text == "/skip":
                thumb = "no"
                await editable.edit("**鈩癸笍 Skipping thumbnail.**")
                await asyncio.sleep(1)
            else:
                await editable.edit("**鈿狅笍 Invalid input! Using default thumbnail.**")
                await asyncio.sleep(1)
        await input6.delete(True)
    except asyncio.TimeoutError:
        await editable.edit("**鈿狅笍 Timeout! Using default thumbnail.**")
        await asyncio.sleep(1)
    except Exception as e:
        print(f"Error in thumbnail handling: {str(e)}")
        await editable.edit("**鈿狅笍 Error! Using default thumbnail.**")
        await asyncio.sleep(1)
    ...
    cmd = (
        f'ffmpeg -headers "Referer: https://classx.co.in" '
        f'-user_agent "Mozilla/5.0" '
        f'-i "{url}" -c copy "{name}.mp4"'
    )
                ...
                while retry_count < max_retries:
                    try:
                        ...
                        if mpd and keys:
                            url = mpd
                            keys_string = " ".join([f"--key {key}" for key in keys])
                            print("鉁� DRM keys fetched!")
                            break
                        
                        print(f"鈿狅笍 Retry {retry_count}/{max_retries}...")
                        await asyncio.sleep(2)  # Reduced wait time
                        
                    except Exception as e:
                        if retry_count >= max_retries:
                            print("鉂� Failed to fetch DRM keys, continuing...")
                            break
                        print(f"鈿狅笍 Retry {retry_count}/{max_retries}...")
                        await asyncio.sleep(2)  # Reduced wait time
                            ...
                            if response.status_code == 200:
                                try:
                                    ...
                                    if url and len(url) > 0:
                                        print(f"鉁� Got signed URL from classplusapp: {url}")
                                        cmd = None  # Don't use yt-dlp for m3u8 files
                                        success = True
                                        continue
                                    else:
                                        print("鈿狅笍 Response JSON does not contain 'data.url'. Here's full response:")
                                        print(json.dumps(res_json, indent=2))
                                except Exception as e:
                                    print("鈿狅笍 Failed to parse response JSON:")
                                    print(response.text)
                                    print("Error:", e)
            ...
            try:
                cc = (
    f"<b>鈹€鈹€鈹€鈹€鈹€鈹€  <i>VID ID </i>: {str(count).zfill(3)}  鈹€鈹€鈹€鈹€鈹€鈹€</b>\n\n"
    f"<b>馃帴 岽浬礇薀岽�</b> : {name1}\n\n"
    f"<blockquote>"
    f"<b>馃挔 蕶岽€岽涐磩蕼 :</b> {b_name}\n"
    f"</blockquote>\n"
    f"<b> 馃摜 岽噚岽浭€岽€岽勧礇岽囜磪 蕶蕪 :</b> {CR}"
)
                cc1 = (
    f"<b>鈹€鈹€鈹€鈹€鈹€鈹€  <i>PDF ID </i>: {str(count).zfill(3)}  鈹€鈹€鈹€鈹€鈹€鈹€</b>\n\n"
    f"<b>馃搼 岽浬礇薀岽�</b> : {name1}\n\n"
    f"<blockquote>"
    f"<b>馃挔 蕶岽€岽涐磩蕼 :</b> {b_name}\n"
    f"</blockquote>\n"
    f"<b> 馃摜 岽噚岽浭€岽€岽勧礇岽囜磪 蕶蕪 :</b> {CR}"
                ...
                ccimg = (
    f"<b>鈹€鈹€鈹€鈹€鈹€鈹€  <i>IMG ID </i>: {str(count).zfill(3)}  鈹€鈹€鈹€鈹€鈹€鈹€</b>\n\n"
    f"<b>馃柤锔� 岽浬礇薀岽�</b> : {name1}\n\n"
    f"<blockquote>"
    f"<b>馃挔 蕶岽€岽涐磩蕼 :</b> {b_name}\n"
    f"</blockquote>\n"
    f"<b> 馃摜 岽噚岽浭€岽€岽勧礇岽囜磪 蕶蕪 :</b> {CR}"
                ...
                if "drive" in url:
                    ...
                    Show = f"<i><b>鈿ast Video Downloading</b></i>\n<blockquote><b>{str(count).zfill(3)}) {name1}</b></blockquote>"
                    prog = await bot.send_message(channel_id, Show, disable_web_page_preview=True)
    ...
    channel_id,
    (
        "<b>鉁� 岽樖€岽忈磩岽囮湵隃� 岽勧磸岽嶀礃薀岽囜礇岽囜磪</b>\n\n"
        "<blockquote><b>馃搶 蕶岽€岽涐磩蕼 纱岽€岽嶀磭 :</b> "
        f"{b_name}</blockquote>\n"
        
        "鈺攢鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€\n"
        f"鈹� 馃敆 岽涐磸岽涐磤薀 岽準€薀隃� : <code>{len(links)}</code>\n"
        f"鈹� 馃煝 隃贬礈岽勧磩岽囮湵隃标湴岽準� : <code>{success_count}</code>\n"
        f"鈹� 鉂� 隃搬磤瑟薀岽囜磪 : <code>{failed_count}</code>\n"
        "鈺扳攢鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€\n\n"

        "鈺攢鈹€鈹€鈹€鈹€鈹€鈹€鈹€ 馃摝 岽勧磤岽涐磭散岽徥€蕪 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€\n"
        f"鈹� 馃帪锔� 岽犐磪岽囜磸隃� : <code>{video_count}</code>\n"
        f"鈹� 馃搫 岽樶磪隃瓣湵 : <code>{pdf_count}</code>\n"
        f"鈹� 馃柤锔� 瑟岽嶀磤散岽囮湵 : <code>{img_count}</code>\n"
        "鈺扳攢鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€\n\n"
        
        "<i>岽噚岽浭€岽€岽勧礇岽囜磪 蕶蕪 岽溕� 蕶岽忈礇隃� 鈿欙笍</i>"
    )
)
